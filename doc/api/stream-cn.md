...
`node:stream` 模块对于创建新类型的流实例很有用.
通常不需要使用 `node:stream` 模块来消费流.

## Organization of this document

本文档包含两个主要部分和第三个注释部分。第一部分解释了如何在应用程序中使用现有的流。第二部分解释如何创建新类型的流.
...
`stream/promises` API 为返回 `Promise` 对象而不是使用回调的流提供了一组替代的异步实用程序函数。该 API 可通过 `require('node:stream/promises')` 或 `require('node:stream').promises` 访问.

### Object mode

Node.js API 创建的所有流都只对字符串和“Buffer”（或“Uint8Array”）对象进行操作。但是，流实现可以使用其他类型的 JavaScript 值（`null` 除外，它在流中具有特殊用途）。这样的流被认为在“对象模式”下运行.
...
可能缓冲的数据量取决于传递给流的构造函数的 `highWaterMark` 选项。对于普通流，`highWaterMark` 选项指定 [total number of bytes][hwm-gotcha]。对于在对象模式下运行的流，`highWaterMark` 指定对象的总数.

当实现调用 [`stream.push(chunk)`][stream-push] 时，数据被缓冲在 `Readable` 流中。如果 Stream 的消费者不调用 [`stream.read()`][stream-read]，则数据将位于内部队列中，直到被消费.

一旦内部读取缓冲区的总大小达到`highWaterMark`指定的阈值，流将暂时停止从底层资源读取数据，直到当前缓冲的数据可以被消费（即流将停止调用内部[` readable._read()`][] 方法，用于填充读取缓冲区）.

当重复调用 [`writable.write(chunk)`][stream-write] 方法时，数据会缓冲在 `Writable` 流中。当内部写入缓冲区的总大小低于 `highWaterMark` 设置的阈值时，调用 `writable.write()` 将返回 `true`。一旦内部缓冲区的大小达到或超过`highWaterMark`，将返回`false`.

`stream` API 的一个关键目标，尤其是 [`stream.pipe()`][] 方法，是将数据缓冲限制在可接受的水平，以便不同速度的源和目标不会压倒可用内存.

`highWaterMark` 选项是一个阈值，而不是限制：它指示流在停止请求更多数据之前缓冲的数据量。它通常不强制执行严格的内存限制。特定的流实现可能会选择执行更严格的限制，但这样做是可选的.

因为 [`Duplex`][] 和 [`Transform`][] 流都是 `Readable` 和 `Writable` 的，所以每个流都维护_two_用于读取和写入的独立内部缓冲区，允许每一方独立操作，同时保持适当和有效的数据流。例如，[`net.Socket`][] 实例是 [`Duplex`][] 流，其 `Readable` 端允许使用从套接字接收的数据，而其 `Writable` 端允许将数据写入 _to_ 套接字。因为数据可能以比接收数据更快或更慢的速率写入套接字，所以每一端都应该独立于另一端操作（和缓冲）.

内部缓冲机制是内部实现细节，可能随时更改。但是，对于某些高级实现，可以使用 `writable.writableBuffer` 或 `readable.readableBuffer` 检索内部缓冲区。不鼓励使用这些未记录的属性.
...
[`Writable`][] 流（例如示例中的 `res`）公开用于将数据写入流的方法，例如 `write()` 和 `end()`.
...
向流中写入数据或从流中消费数据的应用程序不需要直接实现流接口，并且通常没有理由调用 `require('node:stream')`.

希望实现新类型流的开发人员应参考[流实现者的API][]部分.

### Writable streams

可写流是对要写入数据的 _destination_ 的抽象.
...
当流及其任何底层资源（例如文件描述符）已关闭时，会发出“关闭”事件。该事件表示不会再发出任何事件，也不会发生进一步的计算.
...
如果对 [`stream.write(chunk)`][stream-write] 的调用返回 `false`，则当适合继续将数据写入流时将发出 `'drain'` 事件.
...
如果在写入或传输数据时发生错误，则会发出“错误”事件。侦听器回调在调用时被传递一个单一的 `Error` 参数.

除非在创建流时将 [`autoDestroy`][writable-new] 选项设置为 `false`，否则在发出 `'error'` 事件时会关闭流.

在 `'error'` 之后，不应发出除 `'close'` 之外的其他事件（包括 `'error'` 事件）.
...
`'finish'` 事件在 [`stream.end()`][stream-end] 方法被调用后触发，并且所有数据都已刷新到底层系统.
...
当在可读流上调用 [`stream.pipe()`][] 方法时会发出 `'pipe'` 事件，将此可写添加到其目标集.
